...
create_local_gitconfig_from_existing() {
    ...
    local existing_signing_key=""
    local existing_gpgsign=""

    # Do not resolve includes when reading the previous config; this prevents
    # include recursion from breaking extraction on reinstall.
    existing_name=$(run_as_user git config --no-includes --file "$source_file" --get user.name 2>/dev/null || true)
    existing_email=$(run_as_user git config --no-includes --file "$source_file" --get user.email 2>/dev/null || true)
    existing_signing_key=$(run_as_user git config --no-includes --file "$source_file" --get user.signingkey 2>/dev/null || true)
    existing_gpgsign=$(run_as_user git config --no-includes --file "$source_file" --get commit.gpgsign 2>/dev/null || true)

    run_as_user bash -c "cat > '$local_file' << 'EOF'
# Local Git Configuration
# Generated by quick-install.sh from your previous ~/.gitconfig
# Add machine-specific Git settings here.
EOF"

    if [[ -n "$existing_name" ]]; then
        run_as_user git config --file "$local_file" user.name "$existing_name"
    fi
    if [[ -n "$existing_email" ]]; then
        run_as_user git config --file "$local_file" user.email "$existing_email"
    fi
    if [[ -n "$existing_signing_key" ]]; then
        run_as_user git config --file "$local_file" user.signingkey "$existing_signing_key"
    fi
    if [[ -n "$existing_gpgsign" ]]; then
        run_as_user git config --file "$local_file" commit.gpgsign "$existing_gpgsign"
    fi

    echo "Backed up existing .gitconfig to $backup_file"
    echo "Created .gitconfig.local from extracted user settings"
}
